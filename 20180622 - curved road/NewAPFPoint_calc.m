function APF_point = NewAPFPoint_calc(x,y,Carlocal,Num_obCar,obCar,Road)
...
[DisToLane,road_hdg,Roadwidth,DisToRoadCenter,RoadNum,s]=DisToLaneFunction(x,y,Road);
...
APFPoint_road = 20 * (DisToRoadCenter>Roadwidth/2) + 20*exp(-(DisToLane(1))^2/(2*r_eta^2))*(DisToRoadCenter<=Roadwidth/2)+...
                                                     20*exp(-(DisToLane(lane_i))^2/(2*r_eta^2))*(DisToRoadCenter<=Roadwidth/2);
% road_hdg = atan(x/(CurvePoint(2)-y));  %test
...
%   APFPoint_road = (10*(y-(lane(Num_lane)-0.8))^2)*(y>lane(Num_lane)-0.8);

%% 计算环境车辆势场
%相对速度
v_x=Carlocal.State(4)*cos(Carlocal.State(3));
v_y=Carlocal.State(4)*sin(Carlocal.State(3));
...
%     if  obCar(c_i).platoon~=Carlocal.platoon && ~(abs(obCar(c_i).State(2)-Carlocal.State(2))<2  && obCar(c_i).State(1) < Carlocal.State(1)-8 )
    if  obCar(c_i).platoon~=Carlocal.platoon...
        && (obCar(c_i).RoadLocation(3) > Carlocal.RoadLocation(3) || obCar(c_i).RoadLocation(1) > Carlocal.RoadLocation(1))
        %障碍车辆参数初始化
...
APF_point=APFPoint_lane+APFPoint_road;
...
% APF_point=APFPoint_obCar*(APFPoint_obCar>APFPoint_lane)+1/2*(APFPoint_obCar+APFPoint_lane)*(APFPoint_obCar<=APFPoint_lane)+APF_speed+APFPoint_road;
  APF_point=APFPoint_obCar*(APFPoint_obCar>APFPoint_lane)+1/2*(APFPoint_obCar+APFPoint_lane)*(APFPoint_obCar<=APFPoint_lane)+APFPoint_road;
