function APF_point = NewCostFunAPFPoint_calc(x,y,phi,v,Carlocal,Num_obCar,obCar,obCarnow,Num_lane,lane)
...
%   APFPoint_road = (10*(y-(lane(1)+0.5))^2)*(y<lane(1)+0.5)+(10*(y-(lane(Num_lane)-0.5))^2)*(y>lane(Num_lane)-0.5);
...
%         if (vr_x(c_i)>0 ||(obx(c_i)+len(c_i)>x) &&abs(oby(c_i)-Carlocal.State(2))<4.5))
        if (vr_x(c_i)>0) % && ((obx(c_i)+len(c_i)>x)) 
                Ucar=15;
...
%          elseif ((obx(c_i)+len(c_i)>Carlocal.State(1)))
         elseif ((obx(c_i)+len(c_i)>x)) %  && obCar(c_i).HostFlag~=1
