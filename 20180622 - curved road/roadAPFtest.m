...
%仿真道路长度
    ...
    LaneStart=floor(Carhost.State(1)-20);
    LaneEnd=floor(Carhost.State(1)+80);
    [X,Y]=meshgrid(LaneStart:resolution:LaneEnd,Carhost.State(2)-20:resolution:Carhost.State(2)+20);
    ...
    for n_i=1:1:n
        ...
        for m_i=1:1:m  
            ...
            APF_Value_show(m_i,n_i)=NewAPFPoint_calc(X(m_i,n_i),Y(m_i,n_i),Carhost,Num_obCartemp,obCartemp,Road);
...
figure(1);
    ...
    for i_Control=1:Num_Car
        if CarPlot(i_Control).HostFlag==1
            ...
            APFPointValue=NewAPFPoint_calc(Carhost.State(1),Carhost.State(2),Carhost,Num_obCartemp,obCartemp,Road);
            ...
            hold on;
            axis equal;
          %  axis([LaneStart LaneEnd Carhost.State(2)-50 Carhost.State(2)+50 0 37] );
            az = -90;
            el = 7;
            view(az,el);          
        end
